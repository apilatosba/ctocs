...
namespace Main {
   class Program {
      static void Main(string[] args) {
         ...
         {
            Regex functionRegex = new Regex(@"(?<returnType>\w+\s*\**)\s*(?<functionName>\w+)\s*\((?<args>[\w,\s*()\[\]]*?)\)\s*[{;]", RegexOptions.Singleline | RegexOptions.Multiline); // TODO: return type might be more than one word
